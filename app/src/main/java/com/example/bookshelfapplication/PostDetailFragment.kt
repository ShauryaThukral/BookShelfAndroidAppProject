...
class PostDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let{
            ...
            post = args?.postClicked
        }

        post?.let{
            title.text = it.title
            postAuthor.text = "By " + it.createdBy
            time.text ="Posted " + Utility.getTimeAgo(it.createdAt)
            postContent.text = it.description
            price1.text = "For Rs. " + it.price.toString()

            val url = Uri.parse(it.photoUrl)
            Glide.with(requireContext()).load(url).into(books)
